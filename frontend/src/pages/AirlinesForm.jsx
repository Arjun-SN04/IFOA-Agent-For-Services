...
// Aviation-themed cards for "Why IFOA Airlines"
const WHY_CARDS = [
  {
    icon: (
...
export default function AirlinesForm() {
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      await createAirlinesSubscription({ ...formData, paymentStatus: 'pending' })
      setSubmitted(true)
    } catch (e) {
  ...
  return (
    <>
      ...
      <section className="relative h-[85vh] min-h-[520px] flex items-end overflow-hidden">
        <CardImage
          srcs={[
            'https://images.unsplash.com/photo-1529074963764-98f45c47344b?w=1920&q=85&auto=format&fit=crop&crop=focalpoint&fp-y=0.4',
            'https://images.unsplash.com/photo-1464037866556-6812c9d1c72e?w=1920&q=80&auto=format&fit=crop',
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto px-6 sm:px-10 lg:px-16 pb-14 sm:pb-20">
          <motion.div variants={fadeUp} initial="hidden" animate="show" custom={0}
            ...
            <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-4 py-2.5">
              ...
              <span className="text-white/80 text-xs font-bold uppercase tracking-[0.18em]">Airlines & Operators</span>
            </div>
          </motion.div>
          ...
          <motion.p variants={fadeUp} initial="hidden" animate="show" custom={2}
            ...
            Managing a Part 135 operator, airline, or flight school with 3+ certificate holders?<br />
            <span className="text-white font-semibold">We handle FAA compliance for your entire team.</span>
          </motion.p>

          <motion.div variants={fadeUp} initial="hidden" animate="show" custom={3}
            className="flex flex-wrap items-center gap-3 text-sm text-blue-300 font-semibold mb-10">
            {['From $49/cert per year', 'Unlimited lifetime option', '4-step team registration'].map((item, i) => (
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="show" custom={4} className="flex flex-wrap gap-4">
            <button onClick={scrollToForm}
              className="inline-flex items-center gap-2.5 bg-blue-600 hover:bg-blue-500 active:scale-95 text-white font-bold px-8 py-4 rounded-xl text-sm transition-all duration-200 shadow-lg shadow-blue-900/40">
              Start Registration
      ...
      <section className="bg-white py-16 px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
            className="mb-10">
            <p className="text-blue-600 text-xs font-black uppercase tracking-widest mb-2">Why IFOA USA Airlines</p>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}
            ...
            <div className="relative z-10 flex flex-col justify-center px-8 py-8">
              <p className="text-xs font-black uppercase tracking-widest text-blue-400 mb-2">Individual Pilots</p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}
            ...
            <div className="bg-gradient-to-b from-white to-gray-50/80 border-b border-gray-100 px-7 py-7">
              <div className="flex items-start justify-between gap-4 mb-6">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.22em] text-blue-600 mb-1.5">Airlines Registration</p>
            ...
            <div className="px-7 py-8">
              <AnimatePresence mode="wait">
                <motion.div key={step}
                  ...
                  {step === 1 && <AirlinesStep1PlanAndDetails data={formData} update={update} onNext={() => setStep(2)} />}
                  {step === 2 && <AirlinesStep2Holders        data={formData} update={update} onNext={() => setStep(3)} onBack={() => setStep(1)} />}
                  {step === 3 && <AirlinesStep3Preview        data={formData} update={update} onNext={() => setStep(4)} onBack={() => setStep(2)} />}
                  {step === 4 && <AirlinesStep4Payment        data={formData} update={update} onBack={() => setStep(3)} onSubmit={handleSubmit} submitting={submitting} error={error} />}
                </motion.div>
              </AnimatePresence>
            </div>
          </motion.div>
      ...
      <footer className="bg-gray-950 text-gray-400 py-10 px-6">
        ...
        <div className="max-w-6xl mx-auto mt-6 pt-6 border-t border-white/5 text-xs text-gray-600 flex flex-wrap gap-4 justify-between">
          ...
          <div className="flex gap-4">
            ...
            <Link to="/individual/register" className="hover:text-white transition-colors">Individual Plan</Link>
          </div>
        </div>
      </footer>
    </>
