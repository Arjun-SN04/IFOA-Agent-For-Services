...
function Badge({ value, type = 'payment' }) {
  ...
  if (type === 'status') {
    ...
    if (value?.includes('Unlimited'))     { cls += 'bg-blue-50 border-blue-200 text-blue-700'; dot += 'bg-blue-500' }
    else if (value?.includes('Multiple')) { cls += 'bg-violet-50 border-violet-200 text-violet-700'; dot += 'bg-violet-500' }
    ...
    else                           { cls += 'bg-blue-50 border-blue-200 text-blue-600'; dot += 'bg-blue-400' }
  }
...
function SectionHead({ label }) {
  return (
    <p className="text-[10px] font-black uppercase tracking-widest text-blue-600 mb-3 pt-1">{label}</p>
  )
}
          ...
          <div className="px-6 py-5 space-y-6 overflow-y-auto flex-1">
            ...
            <div className="border-t border-slate-100 pt-5"><SectionHead label="Airline / Operator" />
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div className="col-span-2 sm:col-span-3">
                  <ViewField label="Address" value={[record.addressLine1, record.addressLine2, record.city, record.postalCode, record.country].filter(Boolean).join(', ')} />
                </div>
              </div>
            </div>
            <div className="border-t border-slate-100 pt-5"><SectionHead label="Point of Contact" />
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <ViewField label="Name" value={[record.firstName || record.contactFirstName, record.lastName || record.contactLastName].filter(Boolean).join(' ')} />
                ...
                <div className="rounded-xl overflow-hidden border border-slate-200">
                  <table className="w-full text-xs">
                    <thead><tr className="bg-slate-800">
                      {['#', 'Name', 'Cert Type', 'Status', 'FAA #', 'IACRA', 'Email'].map(h => (
                        <th key={h} className="px-3 py-2.5 text-left font-bold uppercase tracking-wide text-white text-[10px]">{h}</th>
                      ))}
                    </tr></thead>
                    <tbody className="divide-y divide-slate-100 bg-white">
                      {record.certificateHolders.map((h, i) => (
                        <tr key={i} className="hover:bg-slate-50">
                          <td className="px-3 py-2.5 font-bold text-blue-600">{i + 1}</td>
                          <td className="px-3 py-2.5 font-semibold text-slate-900 whitespace-nowrap">{h.fullName || '—'}</td>
                          <td className="px-3 py-2.5 text-slate-700">{h.certificateType || '—'}</td>
                          <td className="px-3 py-2.5"><span className={`font-bold text-xs ${h.certificateStatus === 'NEW' ? 'text-emerald-600' : 'text-blue-600'}`}>{h.certificateStatus || '—'}</span></td>
                          <td className="px-3 py-2.5 text-slate-500 font-mono">{h.faaCertificateNumber || '—'}</td>
                          <td className="px-3 py-2.5 text-slate-500 font-mono">{h.iacraFtnNumber || '—'}</td>
                          <td className="px-3 py-2.5 text-slate-500 truncate max-w-[140px]">{h.email || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
        <motion.div initial={{ opacity: 0, y: 16, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }}
          ...
          <div className="flex justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4">
            ...
            <button onClick={handleSave} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 hover:bg-slate-800 px-5 py-2.5 text-sm font-bold text-white transition disabled:opacity-50">
              {saving ? <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-20" /><path fill="currentColor" d="M12 2a10 10 0 0 1 10 10h-4a6 6 0 0 0-6-6V2Z" /></svg> : null}
...
function AirlineEditModal({ record, onClose, onSave, saving }) {
  ...
  useEffect(() => { setForm({ ...record }); setErr('') }, [record])
  const set = (f, v) => setForm(p => ({ ...p, [f]: v }))
  const setHolder = (idx, field, value) => setForm(p => ({ ...p, certificateHolders: p.certificateHolders.map((h, i) => i === idx ? { ...h, [field]: value } : h) }))
  const handleSave = async () => {
    try { setErr(''); await onSave(record._id, form); onClose() }
    catch (e) { setErr(e?.response?.data?.message || 'Save failed.') }
  }
                ...
                <div className="space-y-4">
                  {form.certificateHolders.map((h, idx) => (
                    <div key={idx} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Holder #{idx + 1}</p>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 bg-slate-50">
            {['Name & Certificate', 'Contact', 'Country', 'Plan', 'Price', 'Status', 'Payment', 'Submitted', ''].map(h => (
              <th key={h} className="px-5 py-3.5 text-left text-[10px] font-bold uppercase tracking-widest text-slate-400 whitespace-nowrap">{h}</th>
            ))}
          </tr>
        </thead>
            ...
            <tr key={r._id} className="hover:bg-slate-50/60 transition-colors group cursor-pointer" onClick={() => onView(r)}>
              <td className="px-5 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900 whitespace-nowrap">{[r.firstName, r.middleName, r.lastName].filter(Boolean).join(' ') || '—'}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">{r.primaryCertificate || 'No cert type'}</p>
                  </div>
                </div>
              </td>
              <td className="px-5 py-4">
                <p className="text-slate-700 text-xs truncate max-w-[180px]">{r.email || '—'}</p>
                <p className="text-slate-400 text-[10px] mt-0.5">{r.phone || '—'}</p>
              </td>
              <td className="px-5 py-4 text-slate-600 text-xs whitespace-nowrap">{r.country || '—'}</td>
              <td className="px-5 py-4 whitespace-nowrap"><Badge value={r.subscriptionPlan} type="plan" /></td>
              <td className="px-5 py-4 font-semibold text-slate-900 whitespace-nowrap">{fmtMoney(r.price)}</td>
              <td className="px-5 py-4"><Badge value={r.status} type="status" /></td>
              <td className="px-5 py-4"><Badge value={r.paymentStatus} /></td>
              <td className="px-5 py-4 text-slate-400 text-xs whitespace-nowrap">{fmtDate(r.createdAt)}</td>
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 bg-slate-50">
            {['Airline', 'Contact', 'Country', 'Plan', 'Holders', 'Total', 'Status', 'Payment', 'Submitted', ''].map(h => (
              <th key={h} className="px-5 py-3.5 text-left text-[10px] font-bold uppercase tracking-widest text-slate-400 whitespace-nowrap">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-50">
          {data.map(r => (
            <tr key={r._id} className="hover:bg-slate-50/60 transition-colors group cursor-pointer" onClick={() => onView(r)}>
              <td className="px-5 py-4">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-slate-900 text-white text-xs font-black flex items-center justify-center flex-shrink-0">✈</div>
                  <div>
                    <p className="font-semibold text-slate-900 whitespace-nowrap">{r.airlineName || '—'}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">{r.city || r.addressLine1 || '—'}</p>
                  </div>
                </div>
              </td>
              <td className="px-5 py-4">
                <p className="text-slate-700 text-xs whitespace-nowrap">{[r.firstName, r.lastName].filter(Boolean).join(' ') || [r.contactFirstName, r.contactLastName].filter(Boolean).join(' ') || '—'}</p>
                <p className="text-slate-400 text-[10px] mt-0.5 truncate max-w-[150px]">{r.email || r.contactEmail || '—'}</p>
              </td>
              <td className="px-5 py-4 text-slate-600 text-xs whitespace-nowrap">{r.country || '—'}</td>
              <td className="px-5 py-4 whitespace-nowrap"><Badge value={r.subscriptionPlan} type="plan" /></td>
              <td className="px-5 py-4 text-center">
                <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold">{r.certificateHolders?.length || 0}</span>
              </td>
              <td className="px-5 py-4 font-semibold text-slate-900 whitespace-nowrap">{fmtMoney(r.totalAmount || r.totalServiceFees)}</td>
              <td className="px-5 py-4"><Badge value={r.status} type="status" /></td>
              <td className="px-5 py-4"><Badge value={r.paymentStatus} /></td>
              <td className="px-5 py-4 text-slate-400 text-xs whitespace-nowrap">{fmtDate(r.createdAt)}</td>
              <td className="px-5 py-4" onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => onDelete(r._id, 'airline')} disabled={deleting === r._id} className="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 transition disabled:opacity-40">
                    <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 7h16M10 11v6M14 11v6M6 7l1 12a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2l1-12" /></svg>
                    {deleting === r._id ? '…' : 'Del'}
                  </button>
                </div>
    ...
    <DashboardLayout>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-blue-600">Admin Control Center</p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4 mb-6 shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4 pb-4 border-b border-slate-100">
          <div className="flex flex-wrap gap-2">
            {TAB_CONFIG.map(t => (
              <button key={t.key}
                ...
                className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all ${tab === t.key ? 'bg-slate-900 text-white shadow-sm' : 'border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                {t.label}
                {t.count !== null && (
          ...
          <button onClick={loadData}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 transition">
        ...
        {/* Filter row — only when not overview */}
        {tab !== 'overview' && (
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <circle cx="11" cy="11" r="7" /><path strokeLinecap="round" strokeLinejoin="round" d="m20 20-3.5-3.5" />
              </svg>
            ...
            <a href={tab === 'individuals' ? exportIndividualsExcel() : exportAirlinesExcel()}
              ...
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v10m0 0-4-4m4 4 4-4M4 20h16" />
              </svg>
              Export Excel
            </a>
            ...
            {(search || filterPlan !== 'All' || filterPayment !== 'All' || filterStatus !== 'All') && (
              <button onClick={() => { setSearch(''); setFilterPlan('All'); setFilterPayment('All'); setFilterStatus('All') }}
                className="text-xs text-blue-600 font-semibold hover:underline px-1">
                Clear filters
              </button>
            )}
          </div>
